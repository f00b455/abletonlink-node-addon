    ...
    Napi::Function func =
        DefineClass(env,
                    "MyLink",
                    {
                        ...
                        InstanceMethod("getTimeAtBeat", &MyLink::getTimeAtBeat),
                    });
...
Napi::Value MyLink::getTimeAtBeat(const Napi::CallbackInfo &info)
{
    //throw exception in case of wrong parameter
    ...
    if (this->useAppThreadFunctions)
    {
        time = container.link.captureAppSessionState().timeAtBeat(beat, this->quantum).count();
    }
    else
    {
        time = container.link.captureAudioSessionState().timeAtBeat(beat, this->quantum).count();
    }
